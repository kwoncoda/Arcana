...
async def login_route(
    user_idx: Optional[int] = Query(None, description="현재 로그인 사용자 idx"),
    data_source_idx: Optional[int] = Query(None, description="노션 data_sources.idx"),
):
    """Notion 동의 화면으로 리다이렉트한다."""
    # 예: /login?user_idx=1&data_source_idx=1
    return await notion_login(user_idx, data_source_idx)
