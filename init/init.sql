...
CREATE TABLE rag_indexes (
  ...
  name           VARCHAR(200) NOT NULL             COMMENT '인덱스 이름(예: default)',
  index_type     ENUM('faiss','qdrant','weaviate','pgvector') NOT NULL COMMENT '인덱스 엔진 종류',
  storage_uri    VARCHAR(1000) NULL                COMMENT '파일형(Faiss) 사용 시 저장 경로(S3/로컬)',
  dim            INT NOT NULL                      COMMENT '임베딩 차원 수',
